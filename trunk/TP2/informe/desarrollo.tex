...
\begin{document}
  ...
  \section{Desarrollo}
	...
	\begin{minipage}{0.5\textwidth}
	  ...
	  \begin{algorithmic}
	      \Function{Hebbiano}{$D$, $M$, $Ep_{m}$, $regla$, $\gamma$, $\alpha$}
		\State $Ep = 0$
		\State $N =$ cols(D)
		...
		\While{$Ep < Ep_{max} \land \neg $salir}
		  \For{$x \in D$}
		    \If{$regla = sanger$}
		      \State $\tilde{x} = (W * $triagS$(y'*$unos$(1,M)))$
		      \State $r = unos(M,1) * x - \tilde{x}$
		      \State $\Delta W = \gamma * r .* (y'*$unos$(1,N))$
		    \Else
 		      \State $\Delta W = \gamma * (x-y*W')' * y$
		    \EndIf
		    \State $W += \Delta W$
		  \EndFor
		  
		  \If{$\alpha \not= 0$}
		    \State $\gamma = Ep^{-\alpha}$
		  \EndIf
		  
		  \State suma$\Delta = ||\Delta W||_{Frobenius}$
		  
		  \If{criterio $= p$}
		    \State salir $=$ salidaPesos($suma\Delta$. $\epsilon$)
		  \Else
		    \State salir $=$ salidaOrtogonal($W$. $\epsilon$)
		  \EndIf
		  \State $Ep += 1$
		\EndWhile
		\State \Return $W$
	...
	\begin{minipage}{0.5\textwidth}	
	  ...
	  \begin{algorithmic}
	   \Function{salidaPesos}{$suma\Delta$. $\epsilon$}
	      \If{suma$\Delta < \epsilon$}
		\State \Return true
	      \EndIf
	      \State \Return false
	   \EndFunction
	  ...
	  \begin{algorithmic}
	   \Function{salidaOrtogonal}{$W$. $\epsilon$}
	      \State $j_1 = 1$
	      \While{$j_1 < $\#cols$(W)-1$}
		\State $v_1 = $fila$_{j_1}(W)$
		\State $j_2 = j_1 + 1$
		\While{$j_2 < $\#cols$(W)-1$}
		  \State $v_2 = $fila$_{j_2}(W)$
		  \If{$|v_1' * v_2| > \epsilon$}
		    \State \Return false
		  \EndIf
		  \State $j_2 = j_2 + 1$
	  ...
	  \begin{algorithmic}
		...
		\While{$Ep < Ep_{max}$}
		  \For{$x \in D$}
		    ...
		    \State $\Delta W =$ correccion($Di$, $x'-W$,$\gamma$)
		    \State $W += \Delta W$
		  \EndFor
		  \State $Ep += 1$
		\EndWhile
